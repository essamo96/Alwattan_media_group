    ...
    <div class="card-body">
        ...
        <form role="form" method="post" action="" enctype="multipart/form-data">
            ...
            <div id="video" class="row mb-5 media-type-field media-type-field-video d-none">
                ...
                <div class="col-md-6">
                    <div class="form-check form-check-custom form-check-solid me-5 d-inline-block">
                        <input class="form-check-input video-source-radio" type="radio" name="video_source" id="video_source_url" value="url" {{ $info->video_source != 'file' ? 'checked' : '' }}>
                        <label class="form-check-label" for="video_source_url">رابط خارجي</label>
                    </div>
                    <div class="form-check form-check-custom form-check-solid d-inline-block">
                        <input class="form-check-input video-source-radio" type="radio" name="video_source" id="video_source_file" value="file" {{ $info->video_source == 'file' ? 'checked' : '' }}>
                    ...
                    <div class="mt-3 video-source-field video-source-field-url">
                        <input type="text" value="{{ $info->video_source != 'file' ? $info->video : old('video') }}" name="video" id="video_input" class="form-control" placeholder="https://www.youtube.com/watch?v=...">
                    </div>
                    <div class="mt-3 video-source-field video-source-field-file">
                        <input type="file" name="video_file" class="form-control" accept="video/*">
                        <div class="form-text">اتركه فارغاً للإبقاء على الفيديو الحالي. صيغ مدعومة: mp4, mov, webm, ogg, mkv, avi — بحد أقصى 100 ميجابايت.</div>
                        @if($info->video_source == 'file' && $info->video)
                        <video src="{{ asset($info->video) }}" controls style="margin-top:10px;max-height:150px;display:block;"></video>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div id="media_repeater">
                        ...
                        <div class="row mb-3 align-items-center media-repeater-row" data-index="{{ $loop->index }}">
                            ...
                            <div class="col-md-7">
                                ...
                                @endif
                                <div class="media-row-video-wrap {{ $item->type == 'video' ? '' : 'd-none' }}">
                                    <div class="form-check form-check-custom form-check-solid me-4 d-inline-block">
                                        <input class="form-check-input media-row-video-source" type="radio" name="media_video_source[{{ $loop->index }}]" value="url" {{ $item->video_source != 'file' ? 'checked' : '' }}>
                                        <label class="form-check-label">رابط خارجي</label>
                                    </div>
                                    <div class="form-check form-check-custom form-check-solid d-inline-block">
                                        <input class="form-check-input media-row-video-source" type="radio" name="media_video_source[{{ $loop->index }}]" value="file" {{ $item->video_source == 'file' ? 'checked' : '' }}>
                                        <label class="form-check-label">رفع ملف</label>
                                    </div>
                                    <input type="text" name="media_video_url[{{ $loop->index }}]" value="{{ $item->video_source != 'file' ? $item->video_url : '' }}" class="form-control mt-2 media-row-video-url {{ $item->video_source == 'file' ? 'd-none' : '' }}" placeholder="https://www.youtube.com/watch?v=...">
                                    <input type="file" name="media_video_file[{{ $loop->index }}]" accept="video/*" class="form-control mt-2 media-row-video-file {{ $item->video_source == 'file' ? '' : 'd-none' }}">
                                    @if($item->video_source == 'file' && $item->video_url)
                                    <video src="{{ asset($item->video_url) }}" controls style="margin-top:8px;max-height:80px;display:block;"></video>
