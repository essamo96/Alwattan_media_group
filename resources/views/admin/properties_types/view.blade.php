...
@section('title', 'ادارة انواع العقارات')
...
<li class="breadcrumb-item text-muted"><a href="{{ route('properties_types.view') }}" class="text-muted text-hover-primary">انواع العقارات</a></li>
<li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
<li class="breadcrumb-item text-muted">ادارة انواع العقارات</li>
@endsection
...
<div class="card">
    <div class="card-header border-0 pt-6">
        ...
        @can('admin.properties_types.add')
        <div class="card-toolbar">
            <a href="{{ route('properties_types.add') }}" class="btn btn-primary">
                <i class="ki-duotone ki-plus fs-2"></i> اضافة نوع
            </a>
        </div>
        @endcan
    </div>
    <div class="card-body pt-0">
        ...
        <table class="table align-middle table-row-dashed table-hover fs-6 gy-5" id="properties_types_table">
            <thead>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var oTable = $('#properties_types_table').DataTable({
            "processing": true,
            "serverSide": true,
            "autoWidth": false,
            "language": {
            ...
            "pageLength": 25,
            "ajax": {
                url: "{{ route('properties_types.list') }}",
                data: function (d) {
            ...
            "columns": [
                ...
                {"data": "name_ar", "title": "الاسم", "orderable": true, "searchable": false},
        ...
        $(document).on('click', ".status", function () {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('properties_types.status') }}",
                data: {'id': id}
            }).done(function (data) {
                if (data.type == 'yes') {
        ...
        $(document).on('click', ".delete", function () {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('properties_types.delete') }}",
                data: {'id': id}
            }).done(function (data) {
                toastr[data.status](data.message);
                oTable.draw();
